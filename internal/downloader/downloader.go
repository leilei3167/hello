...
// NewHTTPDownloader 根据线程数量和url来构建一个下载器,根据线程数量进行下载任务分片
func NewHTTPDownloader(url string, par int) (*HTTPDownloader, error) {
	...
	//成功连接得到response 检查是否支持多线程下载
	if resp.Header.Get(acceptRange) == "" { //"Accept-Ranges" 为空的话代表无法根据range来下载
		logrus.Println("此资源不支持多线程下载,将使用单线程模式")
		par = 1
		resumable = false
	}
	//获取文件的大小
	size := resp.Header.Get(contentLength) //"Content-Length" 可能某些资源不会返回文件大小
	if size == "" {
	...
	downloader.DownloadRanges, err = partCalculate(int64(par), //结合文件大小和并发数,分配每个部分的开始结束点
		len, url)
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
// TryConnect 重试3次尝试连接到该url,含有指数退避
...
func partCalculate(par int64, len int64, url string) ([]tool.DownloadRange, error) { //根据线程数,文件大小分配
	...
	if err := tool.Mkdir(folder); err != nil { //创建下载任务目录
		return nil, errors.WithStack(err)
	}

	for i := int64(0); i < par; i++ { //分区策略
		start := (len / par) * i
		end := int64(0)
		if i < par-1 {
			end = (len/par)*(i+1) - 1
		...
		fname := fmt.Sprintf("%s.part%d", file, i) //分区文件的命名
		path := filepath.Join(folder, fname)       //分区文件的绝对路径
